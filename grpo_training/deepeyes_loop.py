...
MODEL_PATH = "/root/local-nvme/models/Lixiang/ChenShawn-DeepEyes-7B"
IMAGE_PATH = "/root/local-nvme/deepeyes/v1/concat.png"
...
def run_deepeyes(image_path: str, question: str) -> Tuple[str, List[str]]:
    ...
    extra_rules = """
                    [当前任务的特别规则，请严格遵守]
                    无论确认与否，每次都在think里尝试回答{yes}或{no}
                    """
    text_qas = question + "\n" + extra_rules + "\n" + PROMPT.USER_PROMPT_V5 #question + PROMPT.USER_PROMPT_V5
    messages: List[dict] = [
        {
            "role": "system",
